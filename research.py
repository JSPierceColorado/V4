...
def _indicators(closes: List[float], volumes: List[float], index: int) -> Dict[str, float]:
    ...
    ret_1d_pct = ((close / closes[index - 1]) - 1) * 100 if index >= 1 and closes[index - 1] else 0.0
    ret_5d_pct = ((close / closes[index - 5]) - 1) * 100 if index >= 5 and closes[index - 5] else 0.0
    ret_10d_pct = ((close / closes[index - 10]) - 1) * 100 if index >= 10 and closes[index - 10] else 0.0
    ret_20d_pct = ((close / closes[index - 20]) - 1) * 100 if index >= 20 and closes[index - 20] else 0.0
    ret_60d_pct = ((close / closes[index - 60]) - 1) * 100 if index >= 60 and closes[index - 60] else 0.0
    avg_volume_20 = _sma(volume_history[:-1], 20)
    volume_ratio = volumes[index] / avg_volume_20 if avg_volume_20 else 0.0
    dollar_vol_m = close * volumes[index] / 1_000_000 if close and volumes[index] else 0.0
    dollar_vol_history = [
        c * v / 1_000_000
        for c, v in zip(history[:-1], volume_history[:-1])
        if c > 0 and v > 0
    ]
    dollar_vol_20d_m = _sma(dollar_vol_history, 20)
    volatility_20_pct = pstdev(recent_returns) * 100 if len(recent_returns) >= 2 else 0.0
    recent_slice = history[-20:] if history else []
    range_20_pct = (
        ((max(recent_slice) / min(recent_slice)) - 1) * 100
        if recent_slice and min(recent_slice) > 0
        else 0.0
    )
    atr_14_pct = (
        mean([abs(history[i] / history[i - 1] - 1) for i in range(max(1, len(history) - 14), len(history)) if history[i - 1] > 0]) * 100
        if len(history) >= 2
        else 0.0
    )
    gap_pct = ret_1d_pct
    return {
        ...
        "dollar_vol_20d_m": dollar_vol_20d_m,
...
def _entry_signal(strategy: Dict[str, Any], closes: List[float], volumes: List[float], index: int) -> tuple[bool, float]:
    ...
    if family == "gap_follow":
        return (
            indicators["gap_pct"] > 1.0
            and close > indicators["sma20"]
            and indicators["volume_ratio"] >= 1.1
        ), score
    if family == "quiet_breakout":
        return (
            indicators["range_20_pct"] < 8.0
            and close >= indicators["high_20"] * 0.995
            and indicators["volume_ratio"] >= 1.0
        ), score
    if family == "liquid_strength":
        return (
            indicators["dollar_vol_20d_m"] >= 10.0
            and indicators["ret_60d_pct"] > 5.0
            and close > indicators["sma50"]
        ), score
...
def _symbol_snapshot(symbol: str, bars: List[Dict[str, Any]]) -> Dict[str, Any] | None:
    ...
    return {
        ...
        "dollar_vol_20d_m": round(indicators["dollar_vol_20d_m"], 3),
        "volatility_20_pct": round(indicators["volatility_20_pct"], 3),
        "atr_14_pct": round(indicators["atr_14_pct"], 3),
        "range_20_pct": round(indicators["range_20_pct"], 3),
...
def generate_web_strategy_variants(
    settings: Settings,
    seed_state: Dict[str, Any],
    bars_by_symbol: Dict[str, Any],
    *,
    ...
    if (
        not settings.autonomy_web_research_enabled
        or not settings.openai_ready
        or max_variants <= 0
    ):
        return []
    ...
    context = {
        ...
        "strategy_dsl": STRATEGY_DSL_GUIDE,
        "task": (
            "Search current market context, then propose paper-trading strategy "
            "candidates that can be represented in the DSL and backtested."
        ),
        "max_strategies": max_variants,
        "recent_research": seed_state.get("last_research"),
        "symbol_snapshots": snapshots,
    ...
    prompt = (
        "You are v4's web-aware trading research strategist. Search the web for "
        "current US equity market regimes, catalysts, sector themes, and risks, then "
        "convert that context into diverse strategy candidates as JSON data. Do not "
        "return executable code. Return only JSON shaped like "
        '{"strategies":[{"name":"...","thesis":"...","entry_logic":"all",'
        '"entry_rules":[{"metric":"ret_20d_pct","op":">","value":3}],'
        '"take_profit_pct":0.04,"stop_loss_pct":-0.025,"max_hold_days":20,'
        '"min_entry_score":20,"score_bias":0}]}.'
    ...
    for index, raw in enumerate((parsed or {}).get("strategies") or [], start=1):
        ...
        if not strategy:
            continue
        strategy["id"] = f"web_{index}_{_slug(raw.get('name') or strategy['id'])}"
        strategy["source"] = "web_strategy_lab"
        strategy["family"] = "web_dsl"
...
def run_research(
    ...
    progress(
        {
            ...
            "message": "Generating web, AI lab, mutation, and coded strategy variants.",
    ...
    web_variants = generate_web_strategy_variants(
        settings,
        state,
        bars,
        max_variants=settings.autonomy_web_strategy_ideas,
    )
    mutation_variants = (
    ...
    deterministic_variants = generate_strategy_variants(
        ...
        max_variants=max(
            0,
            settings.autonomy_research_max_variants
            - len(ai_variants)
            - len(web_variants)
            - len(mutation_variants),
        ),
    )
    generated_variants = ai_variants + web_variants + mutation_variants + deterministic_variants
    progress(
        {
            "stage": "ai_triage",
            "message": (
    ...
    state["last_research"] = {
        ...
        "web_variants_tested": len(web_variants),
    ...
    return {
        ...
        "reply": (
            ...
            f"Web research contributed {len(web_variants)} current-context variants. "
