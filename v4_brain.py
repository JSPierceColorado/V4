...
def rule_parse(message: str) -> Dict[str, Any]:
    ...
    if symbol_research_match and "research status" not in lowered:
        return {
            "action": "symbol_research",
            "args": {"symbol": symbol_research_match.group("symbol").upper()},
        }
    if any(
        phrase in lowered
        for phrase in (
    ...
    if "research" in lowered and any(
        word in lowered for word in ("status", "progress", "done", "finished", "running")
    ):
        return {"action": "research_status", "args": {}}
