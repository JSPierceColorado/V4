    ...
    def start(self) -> Dict[str, Any]:
        ...
        append_event(
            ...
            "research_started",
            {
                ...
                "ai_triage_target": settings.autonomy_ai_variant_triage_target,
...
def summarize_event(event: Dict[str, Any]) -> str:
    ...
    if event_type == "research_started":
        return (
            ...
            f"{payload.get('ai_triage_target', 'n/a')} AI triage target, "
    ...
    if event_type == "market_brief":
        return f"{ts} - market brief\n{payload.get('reply') or payload.get('brief') or 'Recorded.'}"
    if event_type == "symbol_research":
        return (
            f"{ts} - symbol research {payload.get('symbol', '')}\n"
            f"{payload.get('reply') or payload.get('research') or 'Recorded.'}"
        )
...
@app.post("/market/brief", dependencies=[Depends(require_auth)])
def market_brief() -> Dict[str, Any]:
    client = alpaca()
    raw_state = api_result(client.state)
    screen_result = api_result(
        lambda: screen_symbols(
            client,
            settings.autonomy_symbols or None,
            max_symbols_per_cycle=settings.autonomy_screen_symbols_per_cycle,
    ...
    result = build_market_brief(settings, state=raw_state, screen=screen_result)
    append_event(settings.data_dir, "market_brief", result)
    return result


@app.post("/market/symbol/{symbol}", dependencies=[Depends(require_auth)])
def market_symbol(symbol: str) -> Dict[str, Any]:
    client = alpaca()
    raw_state = api_result(client.state)
    result = build_symbol_research(settings, symbol=symbol, state=raw_state)
    append_event(settings.data_dir, "symbol_research", result)
    return result


@app.post("/market/strategy-ideas", dependencies=[Depends(require_auth)])
def market_strategy_ideas() -> Dict[str, Any]:
    client = alpaca()
    raw_state = api_result(client.state)
    screen_result = api_result(
        lambda: screen_symbols(
...
def research() -> Dict[str, Any]:
    ...
    if job["started"]:
        reply = (
            ...
            f"Web research may add up to {settings.autonomy_web_strategy_ideas} current-context ideas. "
...
def order(req: OrderRequest) -> Dict[str, Any]:
    client = alpaca()
    raw_state = api_result(client.state)
    duplicate_reason = duplicate_order_reason(
        raw_state,
        symbol=req.symbol,
        side=req.side,
    )
    if duplicate_reason:
        append_event(
            ...
            {"request": req.model_dump(), "reason": duplicate_reason},
        )
