...
def test_rule_parse_market_brief() -> None:
    parsed = rule_parse("market brief")
    assert parsed["action"] == "market_brief"


def test_rule_parse_symbol_research() -> None:
    parsed = rule_parse("research NVDA")
    assert parsed["action"] == "symbol_research"
    assert parsed["args"]["symbol"] == "NVDA"


def test_rule_parse_strategy_ideas() -> None:
    parsed = rule_parse("search for ideas")
...
def test_normalize_openai_model_aliases() -> None:
    ...
    assert normalize_openai_model("") == "gpt-5.4"
